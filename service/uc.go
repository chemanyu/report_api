...
func UploadUcExcelFiles(ctx *gin.Context) {
	// 获取上传的两个文件
	...
	// 处理Excel文件并执行回调
	result, err := processUcExcelAndCallback(dataSourcePath, clickDataPath)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{
...
// 数据源记录结构
type DataUcSourceRecord struct {
	AdId          string
...
// 打点数据结构
type ClickUcDataRecord struct {
	AdId       string
	ClickCount int
}

// processUcExcelAndCallback 处理Excel文件并执行回调
func processUcExcelAndCallback(dataSourcePath, clickDataPath string) (map[string]interface{}, error) {
	// 1. 读取数据源表（图一）
	dataSourceMap, err := readUcDataSourceExcel(dataSourcePath)
	if err != nil {
		return nil, fmt.Errorf("failed to read data source: %w", err)
	}

	// 2. 读取打点表（图二）
	clickDataList, err := readUcClickDataExcel(clickDataPath)
	if err != nil {
	...
	for _, clickData := range clickDataList {
		...
		for i := 0; i < count; i++ {
			record := records[i]
			// 执行回调
			success := executeUcCallback(record)
			totalCallbacks++
			if success {
				successCallbacks++
...
func readUcDataSourceExcel(filePath string) (map[string][]DataUcSourceRecord, error) {
	...
	// 解析表头，找到各列的索引
	header := rows[0]
	adIdCol := findUcColumnIndex(header, "ad_id")
	callbackParamCol := findUcColumnIndex(header, "callback_param")
	advertiserIdCol := findUcColumnIndex(header, "advertiser_id")
	campaignIdCol := findUcColumnIndex(header, "campaign_id")
	idfaCol := findUcColumnIndex(header, "idfa")
	oaidCol := findUcColumnIndex(header, "oaid")
	oaidSumCol := findUcColumnIndex(header, "oaid_sum")
	imeiCol := findUcColumnIndex(header, "imei")
	imeiSumCol := findUcColumnIndex(header, "imei_sum")
	//reqIdCol := findUcColumnIndex(header, "req_id")
	...
	// 按 ad_id 分组存储数据
	dataMap := make(map[string][]DataUcSourceRecord)
	...
	for i := 1; i < len(rows); i++ {
		...
		if idfaCol != -1 && len(row) > idfaCol {
			record.Idfa = row[idfaCol]
		}
...
func readUcClickDataExcel(filePath string) ([]ClickUcDataRecord, error) {
	f, err := excelize.OpenFile(filePath)
	if err != nil {
		return nil, err
	}
	...
	var clickDataList []ClickUcDataRecord
...
func findUcColumnIndex(header []string, possibleNames ...string) int {
...
func executeUcCallback(record DataUcSourceRecord) bool {
	// 构建回调URL
	baseURL := "https://ad-ocpx.zhltech.net/track/c1d2aa6f57"

	// 构建查询参数
	params := url.Values{}
	params.Set("ms_task", "meishutest")
	params.Set("ms_place", "1")
	params.Set("ms_channel", "Uc")
