...
func UploadHonorExcelFiles(ctx *gin.Context) {
	// 获取上传的两个文件
	...
	// 处理Excel文件并执行回调
	result, err := processHonorExcelAndCallback(dataSourcePath, clickDataPath)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{
...
// 数据源记录结构
type DataHonorSourceRecord struct {
	AdId          string
...
// 打点数据结构
type ClickHonorDataRecord struct {
	AdId       string
	ClickCount int
}

// processGdtExcelAndCallback 处理Excel文件并执行回调
func processHonorExcelAndCallback(dataSourcePath, clickDataPath string) (map[string]interface{}, error) {
	// 1. 读取数据源表（图一）
	dataSourceMap, err := readHonorDataSourceExcel(dataSourcePath)
	if err != nil {
		return nil, fmt.Errorf("failed to read data source: %w", err)
	}

	// 2. 读取打点表（图二）
	clickDataList, err := readHonorClickDataExcel(clickDataPath)
	if err != nil {
	...
	for _, clickData := range clickDataList {
		...
		for i := 0; i < count; i++ {
			record := records[i]
			// 执行回调
			success := sendHonorCallback(record)
			totalCallbacks++
			if success {
				successCallbacks++
...
func readHonorDataSourceExcel(filePath string) (map[string][]DataHonorSourceRecord, error) {
	...
	// 按 ad_id 分组存储数据
	dataMap := make(map[string][]DataHonorSourceRecord)
...
func readHonorClickDataExcel(filePath string) ([]ClickGdtDataRecord, error) {
	f, err := excelize.OpenFile(filePath)
	if err != nil {
		return nil, err
	}
...
func executeHonorCallback(record DataGdtSourceRecord) bool {
	// 构建回调URL
...
func sendHonorCallback(record DataHonorSourceRecord) bool {
	// 构建回调URL
	baseURL := "https://ads-drcn.platform.hihonorcloud.com/api/ad-tracking/v1/conversion?"
